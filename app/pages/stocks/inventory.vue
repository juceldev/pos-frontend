...
const headers = [
  ...
  { title: 'Product Name', key: 'name', sortable: true, minWidth: '380px' },
  ...
  { title: 'Inv-Out', key: 'inv_out_total', sortable: false, align: 'end', minWidth: '120px' },
  { title: 'On Stock', key: 'stock_quantity', sortable: false, align: 'end', minWidth: '90px' },
  { title: 'Category', key: 'category.name', sortable: false, minWidth: '110px' },
  { title: 'Sub-Category', key: 'sub_category.name', sortable: false, minWidth: '150px' },
  { title: 'Seller', key: 'seller', sortable: false, minWidth: '100px' },
  { title: 'Supplier', key: 'supplier_name', sortable: true, minWidth: '100px' },
  { title: 'Inv Updated', key: 'updated_at', sortable: false, minWidth: '150px' },
...
<template>
  <AppPageHeader title="Stocks Inventory" subtitle="All products and services — stock levels and movement summary" />
  ...
  <AppCard class="inventory-card rounded-lg" elevation="1">
    ...
    >
      ...
      <template #item.actions="{ item }">
        <v-btn icon="mdi-eye" variant="text" size="small" @click="viewProduct(item)" />
      </template>
    </AppDataTable>
  </AppCard>
    ...
    <v-card-text v-if="selectedProduct">
      <div class="d-flex flex-column ga-2 mb-4">
        ...
        <div class="d-flex justify-space-between">
          ...
          <span>{{ selectedProduct.product_sequence || '—' }}</span>
        </div>
        <div class="d-flex justify-space-between">
          <span class="text-medium-emphasis">Barcode</span>
        ...
        <div class="d-flex justify-space-between">
          <span class="text-medium-emphasis">Sub Category</span>
          <span>{{ selectedProduct.sub_category?.name ?? '—' }}</span>
        </div>
        <div class="d-flex justify-space-between">
          <span class="text-medium-emphasis">Brand</span>
        ...
        <div class="d-flex justify-space-between">
          <span class="text-medium-emphasis">Unit</span>
          <span>{{ selectedProduct.unit?.name ?? '—' }}</span>
        </div>
        <v-divider class="my-2" />
        <div class="d-flex justify-space-between">
          <span class="text-medium-emphasis">Cost Price</span>
          <span>{{ formatAmount(selectedProduct.cost_price) }}</span>
        </div>
        <div class="d-flex justify-space-between">
          <span class="text-medium-emphasis">Regular Price</span>
          <span>{{ formatAmount(selectedProduct.regular_price) }}</span>
        </div>
        <div class="d-flex justify-space-between">
          <span class="text-medium-emphasis">Stock Quantity</span>
          <span :class="stockStatus(selectedProduct).color + '--text'">{{ Number(selectedProduct.stock_quantity) }}</span>
        </div>
        <div class="d-flex justify-space-between">
          <span class="text-medium-emphasis">Reorder Level</span>
          <span>{{ selectedProduct.reorder_level }}</span>
        </div>
        <div v-if="selectedProduct.warranty_period" class="d-flex justify-space-between">
          <span class="text-medium-emphasis">Warranty</span>
          <span>{{ selectedProduct.warranty_period }}</span>
        </div>
      </div>
    </v-card-text>
    <v-card-actions class="pa-4 pt-0">
      <v-spacer />
      <v-btn variant="text" @click="showProductDialog = false">Close</v-btn>
    </v-card-actions>
  </AppDialog>
