...
const { loading, error, pdfDialog, pdfUrl, pdfTitle, fetchStockInReport, openStockInReportPdf } = useReports()
...
async function load () {
  const response = await fetchStockInReport({
...
<template>
  <div>
    <AppPageHeader title="Stock-In Logs" subtitle="Inbound stock movements by date" />
      ...
      <v-row dense class="pa-3 align-center">
        ...
        <v-col cols="12" md="auto" class="d-flex justify-end gap-2">
          <v-btn
            ...
            size="small"
            @click="openStockInReportPdf(dateFrom, dateTo)"
          >
