...
const customer = computed(() => props.customerName || 'Walk-in')

const quotationNo = `QUO-${String(Date.now()).slice(-8)}`
...
<template>
  <!-- Formal bond-paper quotation layout -->
  <div v-if="isBond" class="receipt-bond">
    ...
    <div class="receipt-bond__masthead">
      ...
      <div class="receipt-bond__masthead-info1">
        <div class="receipt-bond__company-name">QUOTATION</div>
        <div class="receipt-bond__company-line">Quotation No. : {{ quotationNo }}</div>
        <div class="receipt-bond__company-line">Date : {{ quotationDate }}</div>
        <div class="receipt-bond__company-line">Terminal: {{ company?.terminal_id ?? company?.pc_name ?? '' }}</div>
        <div class="receipt-bond__company-line">Customer: {{ customer }}</div>
    ...
    <table class="receipt-bond__items-table">
      ...
      <tbody>
        ...
        <tr v-if="!items.length">
          <td colspan="6" class="text-center" style="color: #999;">No items in cart</td>
        </tr>
      </tbody>
    </table>

    <!-- Bottom: remarks + signatures left, totals right -->
    <div class="receipt-bond__bottom">
      <div class="receipt-bond__payment">
        <div class="receipt-bond__thank-you">
          {{ printer?.receipt_footer_remark ?? 'This quotation is subject to change without prior notice.' }}
        </div>
      ...
      <table class="receipt-bond__totals-table">
        <tbody>
          ...
          <tr>
            ...
            <td class="text-right">{{ discount ? '-' + formatAmount(discount) : formatAmount(0) }}</td>
          </tr>
  ...
  <div v-else class="receipt-thermal">
    ...
    <div class="receipt-thermal__body">
      ...
      <div class="receipt-title text-center">QUOTATION</div>
      <div class="receipt-divider" />
      <div class="receipt-info">
        <div class="receipt-info__row"><span>Quotation No:</span><span>{{ quotationNo }}</span></div>
        <div class="receipt-info__row"><span>Date:</span><span>{{ quotationDate }}</span></div>
      ...
      <div class="receipt-totals">
        ...
        <div v-if="vat" class="receipt-totals__row"><span>VAT</span><span>{{ formatAmount(vat) }}</span></div>
      ...
      <div class="receipt-footer text-center">
        <div class="receipt-footer__remark">{{ printer?.receipt_footer_remark ?? 'This quotation is subject to change without prior notice.' }}</div>
      ...
      <div class="receipt-barcode text-center">
        ...
        <div class="receipt-barcode__text">*{{ quotationNo }}*</div>
      </div>
    </div>
